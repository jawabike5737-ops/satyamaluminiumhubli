...
class MeasurementItem(models.Model):
    ...
    description = models.TextField(blank=True)
    custom_item_name = models.CharField(max_length=200, blank=True, default='')
    ...
    def recalc(self):
        ...
        try:
            subitems = list(self.subitems.all()) if self.pk else []
            if subitems:
                area = sum((sub.area() for sub in subitems), Decimal('0'))
                self.quantity = sum((Decimal(str(sub.quantity or 0)) for sub in subitems), Decimal('0'))
                self.area = area
                self.total = (area * Decimal(self.rate or 0)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                return
            ...
            if w and h:
                ...
                self.area = q
            self.total = (self.area * Decimal(self.rate or 0)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
        except Exception:
