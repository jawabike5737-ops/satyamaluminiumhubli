...
def save_measurements(request, cust_id):
    ...
    with transaction.atomic():
        ...
        incoming_ids = {
            int(item['measurement_item_id']) for item in incoming_items
            if str(item.get('measurement_item_id') or '').isdigit()
        }
        m.items.exclude(id__in=incoming_ids).delete()
        created_ids, updated_ids = [], []

        for item in incoming_items:
            item_id = item.get('measurement_item_id')
            if item_id and str(item_id).isdigit():
                measurement_item = m.items.filter(id=int(item_id)).first()
                if measurement_item is None:
                    return JsonResponse({'error': 'Measurement item does not belong to this customer'}, status=400)
            else:
                measurement_item = MeasurementItem(measurement=m, custom_item_name='')
            ...
            measurement_item.service = service
            measurement_item.service_code = service.service_code if service else ''
            measurement_item.description = (item.get('description') or 'Item').strip()
            measurement_item.custom_item_name = (item.get('custom_item_name') or '').strip()
            measurement_item.item_type = item.get('item_type') or MeasurementItem.SIZE
            measurement_item.unit = item.get('unit') or 'Sq Ft'
            measurement_item.rate = to_decimal(item.get('price_per_unit') or '0').quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
            if measurement_item.pk:
                measurement_item.subitems.all().delete()
            measurement_item.save()

            for sub in item.get('subs') or []:
                def decimal_or_none(value):
                    return None if value in (None, '') else to_decimal(value).quantize(Decimal('0.001'), rounding=ROUND_HALF_UP)

                MeasurementSubItem.objects.create(
                    item=measurement_item,
                    height=decimal_or_none(sub.get('height')),
                    width=decimal_or_none(sub.get('width')),
                    length=decimal_or_none(sub.get('length')),
                    quantity=to_decimal(sub.get('quantity') or sub.get('qty') or '1').quantize(Decimal('0.001'), rounding=ROUND_HALF_UP),
                )

            measurement_item.save()
            (updated_ids if item_id and str(item_id).isdigit() else created_ids).append(measurement_item.id)

        totals = m.items.aggregate(total_qty=Sum('quantity'), total_area=Sum('area'), total_amount=Sum('total'))
        ...
        logger.info(
            'Measurement save customer=%s measurement=%s incoming=%s existing=%s created=%s updated=%s total=%s',
            customer.id, m.id, len(incoming_items), m.items.count(), created_ids, updated_ids, total_amount,
        )
        return JsonResponse({
            'ok': True, 'success': True, 'measurement_id': m.id,
            'total_qty': str(total_qty), 'total_area': str(total_area),
            'total_amount': str(total_amount), 'pdf_url': f'/measurement-pdf/{customer.id}/',
        })
...
def quotation_pdf(request, id):
    ...
    def _build_elements():
        ...
        try:
            ...
            has_terms = (
                _has_text(getattr(q, 'terms_and_conditions', None))
                or qt_qs.exists()
                or _has_text(getattr(q, 'custom_terms', None))
            )
            ...
            if q.terms_and_conditions:
                ...
                # NOTE: do not fall back to printing company default terms
                # when the quotation has no terms selected. The presence of
                # company.terms should not force them into every quotation.
